...
- **YAML 互转** — YAML / JSON / Properties 三格式 6 向互转，自动检测输入格式，用户选择输出格式，代码按格式高亮，点号路径展平/还原，首次输入自动转换
...
- **时间戳** — 时间戳/日期字符串智能互转（多格式解析，无时区按 UTC），select 时间选择器，毫秒/秒/ISO/本地一键复制，UTC±12 世界时区卡片点击复制
